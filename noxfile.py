...
def test(session: nox.Session) -> None:
    ...
    run(f"model=ve effort=20 plot_failed=1 fast=1                 ref='{ROOT}/data/B(H).AlNiCo_5.tab'")
    run(f"model=ve effort=20 plot_failed=1 fast=1 interpolate=300 ref='{ROOT}/data/B(H).LNG37.ansys.tab'")
    run(f"model=ve effort=20 plot_failed=1 fast=1 interpolate=300 ref='{ROOT}/data/Altair_Flux_HystereticExample.csv'")
